...
function initializeOrgchart(data) {
  ...
  var centerNode = function ($node) {
    ...
    var current = getCurrentTranslate($chart);
    ...
    var nodeCenterX = nodeOffset.left - containerOffset.left + ($node.outerWidth() / 2);
    var nodeCenterY = nodeOffset.top - containerOffset.top + ($node.outerHeight() / 2);
    ...
    $chart.css('transform', 'translate(' + newX + 'px, ' + newY + 'px)');
  };
  ...
  $('#chart-container').orgchart({
    ...
    //   return `
    //         <div class="node">
    //             <div class="title" style="white-space: normal; height: auto; padding: 10px;">
    //                 ${data.name}
    //             </div>
    ...
    //       centerNode($(this));
    //     }
    //     else if ($(event.target).is('.bottomEdge, .oci-chevron-down')) {
    //       showDescendents(event.delegateTarget, 3);
    //       centerNode($(event.delegateTarget));
    //     }
    //     else if ($(event.target).is('.topEdge, .oci-chevron-up')) {
    //       centerNode($(event.delegateTarget));
    //     }
    //   });
    // }
...
function fileDisplayContent() {
  ...
  if (fileSize <= 5000000) {
    if (checkFile) {
      ...
      document.getElementById("fileNameSelectedFile").innerHTML =
        "<i style='color:#808080ad;'>Selected File:</i> " +
        ev.target.files[0].name;
    } else {
      alert(
        "Please Insert .docx(word file),.xlsx(excel),.png,.jpg,.jpeg,.gif,.pdf"
      );
...
function progressHandler() {
  ...
  document.getElementById("fileTextUpload").innerHTML =
    Math.round(percent) + "%";
  color = "grey";
  if (percent > 99) {
    document.getElementById("fileTextUpload").innerHTML =
...
function loadDocumentations(dataIdLoad) {
  document.getElementById("listOfDocument").innerHTML =
    "<center><h3 style='color:#00000052'>Loading Files..</h3></center>";
  count = 0;
...
function passwordCheckValid() {
  ...
  if (inputtxt.value.match(passw)) {
    ...
    msgcolor = "red";
    messageVal =
      "Password must contain an uppercase,a number and atleast 6 total digits";
    retypeStatus = true;
  }
...
function checkMatchPass() {
  ...
  if (inputtxt.value == newPass) {
    ...
    msgcolor = "red";
    messageVal =
      "This password must matched with the new password that you enter above";
  }
  _(inputtxt.attributes.msgId.value).style.color = msgcolor;
  _(inputtxt.attributes.msgId.value).innerHTML = messageVal;
...
function changePasswordForm() {
  ...
  if (
    formElements.elements.oldPass.value ===
    formElements.elements.newPassword.value
  ) {
    ...
    xml.onreadystatechange = function () {
      if (this.readyState == 4) {
        if (this.responseText == 1) {
          ...
          _(inputtxt.attributes.msgId.value).innerHTML =
            "<h3>Successful!!</h2>";
        } else {
...
function changeUsernameForm() {
  ...
  xml.onreadystatechange = function () {
    if (this.responseText == 1) {
      ...
      _(formElement.attributes["data-target"].value).innerHTML =
...
function findSuggestion($type, $dataId) {
  ...
  if (ev.keyCode == 13 && ev.srcElement.value.length > 2) {
    document.getElementById("suggestions" + $type + $dataId).innerHTML =
      "<br><center>Fetching Suggestions.....</center>";
    $.post(
      "?config=suggestions",
      {
        type: $type,
...
function ShowMfoList(dat, dept) {
  $("#allModal").modal("setting", "closable", false).modal("show");
  $("#modalContL").html(
    "<div style='text-align: center'><img src='assets/img/loading.gif' style='transform: scale(.1);'></div>"
  );
  setTimeout(() => {
    $.post(
      "?config=rsm",
      ...
      $("#rsm_copy_modal")
        .modal({
          ...
          onApprove: () => {
            ...
            $("#rsm_copy_modal .content").html(
              "<div class='ui active inverted dimmer'><div class='ui text loader'>Copying... please wait...</div></div>"
            );
            $.post(
              "?config=rsm",
              {

<?php
  ...
  if($supEdit){
    $sql = "UPDATE spms_pcr_support_functions SET mfo = '$mfo', suc_in = '$successIndicator', Q = '$quality', E = '$efficiency', T = '$timeliness', percent = '$percent', type = '$pcrType' WHERE spms_pcr_support_functions.id_suppFunc = '$supEdit'";
  }else{
    $sql = "INSERT INTO spms_pcr_support_functions (id_suppFunc, mfo, suc_in, Q, E, T, percent,type)
    ...
    if($_POST['deleteData']){
      $sql = "DELETE FROM spms_pcr_support_functions WHERE spms_pcr_support_functions.id_suppFunc='$_POST[deleteData]'";
      $sql = $mysqli->query($sql);
      if($sql){
        echo 1;
      }else{

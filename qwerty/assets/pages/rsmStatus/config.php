    ...
    while ($dep = $sql->fetch_assoc()) {
        ...
        if ($rsm->num_rows) {
            ...
            $tr .= "<tr>
                        ...
                        <td style='text-align: center;'>
                            <button class='rsm delete button' name='deleteRsm' data-target='$mfoperiod[mfoperiod_id]||$dep[department_id]'>Delete</button>
                        </td>
                   </tr>
            ";
        } else {
...
} elseif (isset($_POST["deleteRsm"])) {
    $period_id = $_POST["period_id"];
    $department_id = $_POST["department_id"];

    if (!$period_id && !$department_id) return false;

    $rsm = new RatingScaleMatrixDestroyer($mysqli);

    $rsm->set_period_id($period_id);
    $rsm->set_department_id($department_id);
